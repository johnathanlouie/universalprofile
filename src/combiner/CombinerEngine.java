 ...
 * 
 *  Output-> FinalC
 *           Entity1 = C1[Entity1]+C2[Entity2]
 *           Entity2 = C1[Entity2]+C2[Entity1]    
 *           Entity3
 * Here Entity1 and Entity2 from C1 and C2 are similar and are combined
 * Similarly, Entity2 and Entity1 from C1 and C2 are similar and are combined
 * to one Entity
 * 
 * SimilarityChecker object is used to check the similarity between every two Entity Objects
 * And SimilarityChecker needs to be given by the user before combining.
 * And the specific Entity object's union(Entity e1, Entity e2); is used to combine similar objects
 * 
 * IMPORTANT: 
 *          There are mainly two things CombinerEngine depends on to work properly.
 *              1)Collections of proper type need to set
 *              2)SimilarityChecker needs to be set
 *  
 * @author sashi
 */
